    ...
    <div className="relative flex w-full max-w-full flex-col items-center px-2 sm:px-16">
      <Card className="relative my-5 w-full max-w-lg rounded-3xl bg-white p-5 shadow-2xl sm:p-10">
        <CardHeader className="mt-0 p-0 text-center">
          ...
          <div className="text-sm font-bold text-indigo-800">***</div>
...
////////////////////// MAIN FORM : SIGNUP //////////////////////
const PasswordResetContainer: React.FC<PasswordResetLinkProps> = ({
  searchParams,
}) => {
  ...
  // const pidUser = searchParams.get('pidUser') as string;
  // const resetCode = searchParams.get('resetCode') as string;
  const pidUser = searchParams.pidUser as string | undefined;
  const resetCode = searchParams.resetCode as string | undefined;
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      const res = await fetch('/api/auth/password-reset-link', {
        ...
        body: JSON.stringify({ password, confirmPassword, pidUser, resetCode }),
      });

      // GET & PROCESS RESPONSE FROM API
      const data: ApiResponse = await res.json();

      if (data.responsex.status == 'PASSWORD_RESET_SUCCESSFUL') {
      ...
      if (data.responsex.status == 'INVALID_RESET') {
        //router.push('/auth/login');
        setMessage(data.responsex.message);
        setLoading(false);
      }

      if (data.responsex.status == 'PASSWORD_NOT_COMPLEX') {
        //router.push('/auth/login');
        setMessage(data.responsex.message);
        setLoading(false);
      }

      if (data.responsex.status == 'PASSWORD_NOT_SAME') {
        //router.push('/auth/login');
        setMessage(data.responsex.message);
        setLoading(false);
      }
