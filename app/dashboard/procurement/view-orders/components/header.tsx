...
function Header({ createControl, statusControl }: { createControl?: React.ReactNode; statusControl?: React.ReactNode }) {
  return (
    <div className="px-3 py-[25px] dark:bg-black">
      <div className="flex flex-col dark:bg-black sm:flex-row sm:items-center sm:justify-between">
        ...
        {createControl ?? <CreateOrder />}
      </div>
      <div>
        {statusControl ?? <OrderTypes />}
      </div>
    </div>
  );
}
