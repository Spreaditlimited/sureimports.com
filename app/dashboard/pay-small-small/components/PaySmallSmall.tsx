...
const CheckboxIcon = React.memo(({ checked }: { checked: boolean }) => (
  <div className="relative flex h-5 w-5 shrink-0 cursor-pointer items-center justify-center rounded-[6px] border-2 transition-colors duration-200">
    <div
      className={`absolute inset-0 rounded-[4px] transition-colors ${checked ? 'border-blue-600 bg-blue-600' : 'border-slate-300 bg-transparent dark:border-slate-600'}`}
    />
    {checked && (
      <svg
...
function WalletPaymentDialog({
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="overflow-hidden rounded-[32px] border-0 bg-white p-0 shadow-2xl dark:bg-slate-900 sm:max-w-md">
        ...
        <div className="space-y-6 p-6">
          ...
          <div className="space-y-3">
            ...
            <div
              className={`flex items-center justify-between rounded-xl p-3 ${hasInsufficientFunds ? 'bg-rose-50 dark:bg-rose-900/20' : 'bg-slate-50 dark:bg-slate-800/50'}`}
            >
              <span className="text-sm font-medium text-slate-600 dark:text-slate-400">
                Remaining Balance
              </span>
              <span
                className={`font-bold ${hasInsufficientFunds ? 'text-rose-600' : 'text-slate-900 dark:text-white'}`}
              >
          ...
          <div className="flex gap-3 pt-2">
            ...
            <Button
              ...
              className="flex-1 rounded-xl bg-emerald-600 text-white shadow-lg shadow-emerald-600/20 hover:bg-emerald-500"
            >
...
export default function App({
  ...
  const startPaySmallSmall = async (
    ...
    if (statusx == 'NO_ACCOUNT' || statusx == 'NO_CUSTOMER') {
      toast.warning('Please activate your wallet to start paying small small.');
      return;
    }
    if (transactions.totalAmount < 5000) {
      toast.warning('Minimum wallet balance of N5,000 required to activate.');
      return;
    }
    try {
      setLoading(true);
      const res = await fetch(
        `/api/pay-small-small/start?pidUser=${pidUser}&pidPaySmallSmall=${pidPaySmallSmall}&pidProduct=${pidProduct}`,
      );
  ...
  const cancelPaySmallSmall = async (
    ...
    try {
      const res = await fetch(
        `/api/pay-small-small/cancel?pidUser=${pidUser}&pidPaySmallSmall=${pidPaySmallSmall}&pidProduct=${pidProduct}`,
      );
      const data = await res.json();
      if (data.statusx == 'SUCCESS') {
        toast.success(data.message);
  ...
  const handleWalletPaymentConfirmed = async () => {
    ...
    try {
      ...
      if (data.statusx === 'SUCCESS') {
        ...
        toast.warning(data.message || 'Claim failed.');
      }
    } catch (error) {
      setIsWalletPaymentProcessing(false);
      setShowWalletDialog(false);
  ...
  const handleCancelConfirm = async () => {
    ...
    await cancelPaySmallSmall(
      cancellingProductId[0],
      cancellingProductId[2],
      cancellingProductId[1],
      cancellingProductId[3],
      cancellingProductId[4],
    );
    setProducts(
      products.map((p) =>
        p.id === cancellingProductId[0]
          ? { ...p, status: 'CANCELLED', checked: false }
          : p,
      ),
    );
    ...
    <div className="min-h-screen bg-[#fcfcfd] dark:bg-slate-950">
      ...
      <div className="bg-slate-900 pb-32 pt-12 text-white">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <p className="mt-2 text-slate-400">
            Lock down your favorite products and pay at your own pace.
          </p>
        </div>
      </div>
      ...
      <main className="mx-auto -mt-16 max-w-7xl px-4 pb-20 sm:px-6 lg:px-8">
        ...
        <div className="mb-8 grid grid-cols-1 gap-6 lg:grid-cols-3">
          ...
          <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900 sm:p-8 lg:col-span-2">
            <h2 className="mb-6 flex items-center gap-2 text-sm font-bold uppercase tracking-wider text-slate-500">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div className="rounded-2xl bg-slate-50 p-4 dark:bg-slate-800/50">
                ...
                <p className="font-bold text-slate-900 dark:text-white">
                  {customer?.bankName || 'Not Assigned'}
                </p>
              </div>
              <div className="rounded-2xl bg-slate-50 p-4 dark:bg-slate-800/50">
                <div className="mb-1 flex items-center gap-2 text-slate-400">
                ...
                <p className="font-mono text-lg font-bold tracking-wider text-slate-900 dark:text-white">
          ...
          <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-blue-600 to-indigo-700 p-8 text-white shadow-xl shadow-blue-900/20">
            ...
            <div className="relative z-10 flex h-full flex-col justify-between">
              <div>
                <p className="mb-1 flex items-center gap-2 text-sm font-bold uppercase tracking-wider text-blue-200">
                  <Coins className="h-4 w-4" /> Available Balance
                </p>
                <h3 className="mt-2 text-4xl font-black">
                  ₦
                  {Number(transactions?.totalAmount ?? 0).toLocaleString(
                    undefined,
              ...
              <div className="mt-8 flex items-center gap-2">
                <span
                  className={`flex h-2 w-2 rounded-full ${statusx === 'WALLET_READY' ? 'animate-pulse bg-emerald-400' : 'bg-rose-400'}`}
                ></span>
        ...
        {/* Tab Navigation */}
        <div className="no-scrollbar mb-8 flex items-center gap-2 overflow-x-auto rounded-2xl border border-slate-200 bg-white p-2 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          {TABS.map((tab) => (
            <button
              key={tab.id}
              onClick={() => setSelectedStatus(tab.id)}
              className={`flex min-w-[120px] flex-1 items-center justify-center gap-2 rounded-xl px-4 py-3 text-sm font-bold transition-all ${
            ...
            >
              ...
              {tab.label}
              <span
                className={`ml-1 rounded-full px-2 py-0.5 text-[10px] ${selectedStatus === tab.id ? 'bg-white/20' : 'bg-slate-100 dark:bg-slate-800'}`}
              >
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {filteredProducts.map((product: any) => {
            const canClaim =
              transactions?.totalAmount >= parseFloat(product.amount);
            const showCheck =
            ...
            return (
              <div
                key={product.id}
                className="flex flex-col overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm transition hover:shadow-lg dark:border-slate-800 dark:bg-slate-900"
              >
                {/* Image Section */}
                <div className="relative flex aspect-square w-full items-center justify-center bg-slate-50 p-6 dark:bg-slate-800/50">
                  <img
                    src={
                      resolveMediaUrl(
                        product?.store?.productImage ?? product?.productImage,
                      ) || '/placeholder.png'
                    }
                  ...
                  {product.status === 'COMPLETED' && (
                    <div className="absolute right-4 top-4 rounded-full bg-emerald-500 px-3 py-1 text-[10px] font-black uppercase tracking-widest text-white shadow-md">
                      Paid Off
                    </div>
                  )}
                </div>
                  ...
                  <div className="mt-4 flex-1">
                    ...
                    {showCheck && (
                      <label
                        ...
                        className="mt-4 flex cursor-pointer items-start gap-3 rounded-xl bg-slate-50 p-3 dark:bg-slate-800/50"
                      >
                        <input
                        ...
                        <span className="text-xs font-medium leading-relaxed text-slate-600 dark:text-slate-300">
                          {product.status === 'SAVED'
                            ? 'Select to lock this price and start installment plan.'
                            : 'Select to cancel your installment plan for this item.'}
                        </span>
                      </label>
                    )}
                  </div>
                  ...
                  <div className="mt-6 space-y-3">
                    {product.status === 'SAVED' && (
                      <Button
                        ...
                        className="h-12 w-full rounded-xl bg-slate-900 font-bold text-white hover:bg-slate-800 dark:bg-blue-600 dark:hover:bg-blue-500"
                      >
                        Activate Plan
                      </Button>
                    )}
                    ...
                    {product.status === 'STARTED' && (
                      <>
                        <Button
                          ...
                          disabled={!canClaim}
                          className="h-12 w-full rounded-xl bg-emerald-600 font-bold text-white shadow-lg shadow-emerald-600/20 hover:bg-emerald-500"
                        >
                          {canClaim
                            ? 'Claim Product Now'
                        ...
                        <Button
                          ...
                          variant="outline"
                          disabled={!product.checked}
                          className="h-12 w-full rounded-xl border-rose-200 font-bold text-rose-600 hover:bg-rose-50 hover:text-rose-700 dark:border-rose-900/50 dark:hover:bg-rose-900/20"
                        >
                          Cancel Plan
                        </Button>
                      </>
        ...
        {filteredProducts.length === 0 && (
          <div className="mt-6 flex flex-col items-center justify-center rounded-3xl border border-dashed border-slate-300 bg-white py-24 text-center dark:border-slate-700 dark:bg-slate-900">
            <div className="rounded-full bg-slate-50 p-6 dark:bg-slate-800">
              <Package className="h-12 w-12 text-slate-300" />
            </div>
            <h3 className="mt-6 text-xl font-bold text-slate-900 dark:text-white">
              No {selectedStatus.toLowerCase()} items
            </h3>
            <p className="mx-auto mt-2 max-w-sm text-slate-500">
              You don't have any products in this category yet. Browse the store
              to find items you love.
            </p>
            {selectedStatus === 'SAVED' && (
              <button
                onClick={() => router.push('/shop')}
                className="mt-8 flex items-center gap-2 rounded-xl bg-blue-600 px-8 py-3 font-bold text-white shadow-lg transition hover:bg-blue-500"
              >
      ...
      <Dialog open={showCancelDialog} onOpenChange={setShowCancelDialog}>
        <DialogContent className="rounded-[32px] p-6 sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-xl font-bold text-slate-900 dark:text-white">
          ...
          <div className="py-4">
            <p className="text-sm leading-relaxed text-slate-600 dark:text-slate-400">
              Discontinuing this plan will automatically deduct a{' '}
              <span className="font-bold text-slate-900 dark:text-white">
                2.5% administrative fee
              </span>{' '}
              from the amount you've paid so far. The remaining balance will be
          ...
          <div className="flex gap-3">
            ...
            <Button
              ...
              disabled={isProcessing}
              className="flex-1 rounded-xl bg-rose-600"
            >
...
const CountdownTimer = React.memo(({ startDate }: { startDate: string }) => {
    ...
    return (
      <div className="rounded-xl border border-rose-200 bg-rose-50 p-3 text-center dark:border-rose-900/30 dark:bg-rose-900/10">
        <p className="flex items-center justify-center gap-2 text-xs font-bold text-rose-600">
          <XCircle className="h-4 w-4" /> Plan Expired
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-xl border border-blue-100 bg-blue-50/50 p-3 dark:border-blue-900/30 dark:bg-blue-900/10">
      <p className="mb-2 flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider text-blue-600">
      ...
      <div className="flex items-center justify-between gap-1">
        {[
          ...
          <div
            key={idx}
            className="flex flex-1 flex-col items-center justify-center rounded-lg border border-slate-100 bg-white p-2 shadow-sm dark:border-slate-700 dark:bg-slate-800"
          >
