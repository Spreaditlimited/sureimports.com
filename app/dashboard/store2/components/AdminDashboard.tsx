...
import { ArrowLeft, Plus, Edit, Trash2, Eye, TrendingUp, TrendingDown, Package, Users, DollarSign, ShoppingCart, AlertTriangle, UserPlus, RotateCcw, Search, Filter, X, LogOut, Menu, BarChart3, PieChart, Download, Activity, Target, Settings } from 'lucide-react';
...
import { StoreSettings, SettingsManager, getStoreSettings } from './admin/SettingsManager';
...
export function AdminDashboard({
  ...
  const [settingsManager] = useState(() => new SettingsManager(storeSettings, setStoreSettings));
    ...
    allOrders.forEach(order => {
      if (order.customerDetails) {
        ...
        if (existingCustomer) {
          ...
          if (new Date(order.orderDate) > new Date(existingCustomer.lastOrderDate)) {
          ...
          customerMap.set(order.customerDetails.email, {
            ...
            preferredCategory: ['Laptops', 'Phones', 'Tablets', 'Accessories'][Math.floor(Math.random() * 4)]
          });
        }
      }
    });
  ...
  const exportProducts = () => {
    const headers = ['id', 'title', 'description', 'price', 'supplierPrice', 'affiliateCommission', 'brand', 'category', 'condition', 'rating', 'storage', 'memory', 'warranty'];
  ...
  const exportOrders = () => {
    const headers = ['orderNumber', 'productName', 'customerName', 'customerEmail', 'customerPhone', 'quantity', 'price', 'status', 'orderDate', 'expectedDelivery', 'isBulkOrder'];
  ...
  const exportCustomers = () => {
    const headers = ['id', 'name', 'email', 'phone', 'address', 'totalOrders', 'totalSpent', 'lastOrderDate', 'status', 'preferredCategory'];
  ...
  const exportAnalytics = () => {
    ...
    const exportData = [
      ...
      ...salesData.map(data => ({ metric: `${data.month} Sales`, value: data.sales })),
      ...salesData.map(data => ({ metric: `${data.month} Revenue (₦)`, value: data.revenue })),
      ...categoryData.map(data => ({ metric: `${data.name} Products (%)`, value: data.value }))
    ];
  ...
  const calculateStorePrice = (supplierPrice: number, category: string) => {
    const formula = storeSettings.pricingFormulas[category] || storeSettings.pricingFormulas['Phones'];
    const storePriceNaira = (supplierPrice + formula.addition) * formula.multiplier;
  ...
  useEffect(() => {
    if (newProduct.supplierPrice && newProduct.category) {
      const autoCalculatedPrice = calculateStorePrice(newProduct.supplierPrice, newProduct.category);
      const autoCalculatedCommission = getDefaultCommission(newProduct.category);
  ...
  useEffect(() => {
    if (editingProduct && editingProduct.supplierPrice && editingProduct.category) {
      const autoCalculatedPrice = calculateStorePrice(editingProduct.supplierPrice, editingProduct.category);
      const autoCalculatedCommission = getDefaultCommission(editingProduct.category);
  ...
  const getAvailableTabs = () => {
    ...
    if (hasPermission('products')) {
      tabs.push({ value: 'products', label: 'Product Management', icon: Package });
    }
    if (hasPermission('orders')) {
      tabs.push({ value: 'orders', label: 'Order Management', icon: ShoppingCart });
    }
    if (hasPermission('customers')) {
      tabs.push({ value: 'customers', label: 'Customer Management', icon: Users });
    }
    if (hasPermission('reports')) {
      tabs.push({ value: 'analytics', label: 'Analytics & Reports', icon: PieChart });
    }
          ...
          <div className="flex items-center justify-between mb-8">
            ...
            <div className="flex items-center gap-3">
              <div className="text-right">
                ...
                <p className="font-medium text-foreground">{currentAdmin?.name}</p>
              </div>
  ...
  const handleAddProduct = () => {
    if (newProduct.title && newProduct.supplierPrice && newProduct.category && newProduct.brand) {
      const product: Product = {
        ...
        image: newProduct.image || 'https://images.unsplash.com/photo-1560472354-b33ff0c44a43?w=500&h=500&fit=crop&crop=center',
        ...
        graphicsMemory: newProduct.graphicsMemory === 'none' ? undefined : newProduct.graphicsMemory,
  ...
  const filteredProducts = useMemo(() => {
    return products.filter(product => {
      ...
      const matchesBrand = productBrandFilter === "All" || product.brand === productBrandFilter;
      const matchesCondition = productConditionFilter === "All" || product.condition === productConditionFilter;
      
      return matchesSearch && matchesCategory && matchesBrand && matchesCondition;
    });
  }, [products, productSearchQuery, productCategoryFilter, productBrandFilter, productConditionFilter]);
  ...
  const filteredOrders = useMemo(() => {
    ...
    return allOrders.filter(order => {
      const matchesSearch = orderSearchQuery === "" || 
        ...
        order.product.name.toLowerCase().includes(orderSearchQuery.toLowerCase()) ||
        order.customerDetails?.name.toLowerCase().includes(orderSearchQuery.toLowerCase()) ||
        order.customerDetails?.email.toLowerCase().includes(orderSearchQuery.toLowerCase());
  ...
  const productBrands = ['All', ...Array.from(new Set(products.map(p => p.brand)))];
  const productConditions = ['All', ...Array.from(new Set(products.map(p => p.condition)))];
  
  const orderStatuses = ['All', 'confirmed', 'shipped', 'arrived', 'delivered', 'cancelled', 'replaced', 'shipped-back', 'arrived-back'];
            ...
            <div className="flex items-center justify-between h-14 py-2">
              <div className="flex items-center gap-2 min-w-0 flex-1">
                ...
                <div className="min-w-0 flex-1">
                  <h1 className="text-sm font-medium truncate">Admin Dashboard</h1>
              ...
              <div className="flex items-center gap-1 flex-shrink-0">
                {hasPermission('userManagement') && currentAdmin.role === 'super_admin' && (
                  <Button
            ...
            <div className="bg-card border border-border rounded-lg overflow-hidden">
              ...
              <div className="flex items-center justify-between p-3 bg-muted/30 border-b border-border">
                ...
                <div className="text-xs text-muted-foreground bg-muted/50 px-2 py-1 rounded">
                  {availableTabs.findIndex(tab => tab.value === activeTab) + 1} of {availableTabs.length}
                </div>
              </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="truncate text-left">
                        {tab.label.replace(' Management', '').replace(' & Reports', '')}
                      </span>
                    </Button>
                  );
                })}
                ...
                <CardContent>
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Select value={productConditionFilter} onValueChange={setProductConditionFilter}>
                        <SelectTrigger>
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
                ...
                <CardContent>
                      ...
                      <TableBody>
                        {filteredProducts.slice(0, 10).map((product) => (
                          <TableRow key={product.id}>
                            ...
                            <TableCell>₦{product.affiliateCommission.toLocaleString()}</TableCell>
                            <TableCell>
                              <div className="flex items-center gap-2">
                                <Button
                                  variant="outline"
                  ...
                  {filteredProducts.length > 10 && (
                    <div className="mt-4 text-center">
                      <p className="text-sm text-muted-foreground">
                        Showing 10 of {filteredProducts.length} products. Export to see all data.
                      </p>
                    </div>
                  )}
                </CardContent>
            ...
            <TabsContent value="orders" className="space-y-6 mt-0">
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                ...
                <div className="flex flex-col sm:flex-row gap-2">
                  ...
                  <Button onClick={() => setIsCreatingBulkOrder(true)} className="flex items-center gap-2">
                    <Plus className="h-4 w-4" />
                    Create Bulk Order
                  </Button>
                </div>
              ...
              <Card>
                ...
                <CardContent>
                          ...
                          <TableRow key={order.id}>
                            ...
                            <TableCell>
                              <div className="flex items-center gap-3">
                                ...
                                />
                                <span className="text-sm line-clamp-1">{order.product.name}</span>
                              </div>
                            </TableCell>
                            <TableCell>
                              <div>
                            ...
                            <TableCell>
                              <Badge variant={order.isBulkOrder ? 'secondary' : 'outline'}>
                  ...
                  {filteredOrders.length > 10 && (
                    <div className="mt-4 text-center">
                      <p className="text-sm text-muted-foreground">
                        Showing 10 of {filteredOrders.length} orders. Export to see all data.
                      </p>
                    </div>
                  )}
                </CardContent>
              ...
              <Card>
                ...
                <CardContent>
                      ...
                      <TableBody>
                        {filteredCustomers.slice(0, 10).map((customer) => (
                          <TableRow key={customer.id}>
                            ...
                            <TableCell>
                              <Badge variant="outline">{customer.preferredCategory}</Badge>
                            </TableCell>
                            <TableCell>
                              <Button
                                variant="outline"
                  ...
                  {filteredCustomers.length > 10 && (
                    <div className="mt-4 text-center">
                      <p className="text-sm text-muted-foreground">
                        Showing 10 of {filteredCustomers.length} customers. Export to see all data.
                      </p>
                    </div>
                  )}
                </CardContent>
                          ...
                          <div className="flex items-center gap-2">
                            <div className="w-20 bg-muted rounded-full h-2">
                              <div 
                                ...
                                style={{ width: `${(data.sales / 300) * 100}%` }}
                              />
                            </div>
                ...
                <CardContent>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="text-center">
                      <div className="text-2xl font-bold text-green-600">₦{(stats.totalRevenue / stats.totalOrders).toLocaleString()}</div>
                      <p className="text-sm text-muted-foreground">Average Order Value</p>
                    </div>
                    <div className="text-center">
                      <div className="text-2xl font-bold text-blue-600">{((stats.totalOrders - stats.pendingOrders) / stats.totalOrders * 100).toFixed(1)}%</div>
                      <p className="text-sm text-muted-foreground">Order Completion Rate</p>
                    </div>
                    <div className="text-center">
                      <div className="text-2xl font-bold text-purple-600">{(stats.totalOrders / stats.totalCustomers).toFixed(1)}</div>
