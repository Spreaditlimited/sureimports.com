...
export class SettingsManager {
  ...
  editCategory(oldName: string, newName: string): boolean {
    ...
    if (
      ...
      this.settings.categories[trimmedNewName]
    ) {
      return false;
    }
  ...
  addBrand(category: string, brandName: string): boolean {
    ...
    if (
      !trimmedName ||
      this.settings.categories[category].includes(trimmedName)
    ) {
      return false;
    }
  ...
  deleteBrand(category: string, brand: string): void {
    const newSettings = {
      ...
      categories: {
        ...this.settings.categories,
        [category]: this.settings.categories[category].filter(
          (b) => b !== brand,
        ),
      },
    };
  ...
  editBrand(category: string, oldBrand: string, newBrand: string): boolean {
    ...
    if (
      !trimmedNewBrand ||
      trimmedNewBrand === oldBrand ||
      this.settings.categories[category].includes(trimmedNewBrand)
    ) {
      return false;
    }
    ...
    const newSettings = {
      ...
      categories: {
        ...this.settings.categories,
        [category]: this.settings.categories[category].map((b) =>
          b === oldBrand ? trimmedNewBrand : b,
        ),
      },
    };
  ...
  editListFieldValue(
    ...
    if (
      !trimmedNewValue ||
      trimmedNewValue === oldValue ||
      currentList.includes(trimmedNewValue)
    ) {
      return false;
    }
