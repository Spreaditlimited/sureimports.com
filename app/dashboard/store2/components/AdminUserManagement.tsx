...
export function AdminUserManagement({
  ...
  const [localSelectedAdmin, setLocalSelectedAdmin] =
  ...
  if (!isSuperAdmin) {
    return (
      <div className="min-h-screen bg-background">
        ...
        <div className={`${isMobile ? 'px-5 py-4' : 'mx-auto max-w-7xl p-6'}`}>
          <Card>
            <CardHeader>
  ...
  return (
    <div className="min-h-screen bg-background">
            ...
            <div className="flex h-16 items-center justify-between">
              ...
              <div className="flex items-center gap-3">
                ...
                {onBulkBuyer && (
                  <button
                    onClick={onBulkBuyer}
                    className="flex items-center gap-2 rounded-lg bg-purple-600 px-3 py-1.5 text-sm text-white transition-colors hover:bg-purple-700 dark:bg-purple-500 dark:hover:bg-purple-600"
                  >
      ...
      {/* Main Content */}
      <div
        className={`${isMobile ? 'px-5 py-4 pb-6' : 'mx-auto max-w-7xl p-6'}`}
      >
        <div className="space-y-6">
          ...
          {/* Stats Cards */}
          <div
            className={`grid ${isMobile ? 'grid-cols-1 gap-3' : 'grid-cols-1 gap-4 md:grid-cols-3'}`}
          >
            <Card>
              ...
              <CardContent>
                <div
                  className={`${isMobile ? 'text-xl' : 'text-2xl'} font-medium`}
                >
                  {admins.length}
                </div>
          ...
          <Card>
            ...
            <CardContent className={`${isMobile ? 'p-3' : 'space-y-4'}`}>
              {admins.map((admin, index) => (
                <div key={admin.id}>
                  <div
                    className={`${isMobile ? 'space-y-3' : 'flex items-center justify-between'} rounded-lg border p-4`}
                  >
                    <div
                      ...
                      <div className={`space-y-1 ${isMobile ? 'flex-1' : ''}`}>
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          {admin.role === 'super_admin' && (
                            <Badge
                              className={`bg-gold-100 text-gold-800 border-gold-200 ${isMobile ? 'text-xs' : ''}`}
                            >
                              <ShieldCheck
                                className={`${isMobile ? 'h-2 w-2' : 'h-3 w-3'} mr-1`}
                              />
                              Super Admin
                            </Badge>
                          )}
                        ...
                        <div
                          className={`flex items-center gap-4 ${isMobile ? 'text-xs' : 'text-xs'} text-muted-foreground`}
                        >
                    ...
                    <div
                      className={`${isMobile ? 'flex justify-end' : 'flex items-center gap-3'}`}
                    >
                      {!isMobile && (
                      ...
                      <div
                        className={`flex gap-2 ${isMobile ? 'flex-wrap' : ''}`}
                      >
                        {admin.role !== 'super_admin' && (
                          <>
                            <Button
                            ...
                            >
                              <Edit
                                className={`${isMobile ? 'h-3 w-3' : 'h-4 w-4'} ${isMobile ? '' : 'mr-1'}`}
                              />
              ...
              {admins.length === 0 && (
                <div className="py-8 text-center">
                  ...
                  <h3 className="mb-2 font-medium">No administrators found</h3>
      ...
      >
        <DialogContent
          className={`${isMobile ? 'mx-4 max-w-sm' : 'max-w-2xl'}`}
        >
          <DialogHeader>
            ...
            <DialogDescription className={`${isMobile ? 'text-xs' : ''}`}>
              Configure dashboard access permissions for{' '}
              {localSelectedAdmin?.name}
            </DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <DialogContent className={`${isMobile ? 'mx-4 max-w-sm' : ''}`}>
          <DialogHeader>
            <DialogTitle
              className={`flex items-center gap-2 text-destructive ${isMobile ? 'text-base' : ''}`}
            >
              <AlertTriangle className="h-5 w-5" />
            ...
            <DialogDescription className={`${isMobile ? 'text-xs' : ''}`}>
              ...
              admin account for <strong>{adminToDelete?.name}</strong> and
              remove all their access permissions.
            </DialogDescription>
          </DialogHeader>
