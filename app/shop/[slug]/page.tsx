...
function ProductDetailsContent({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const fetchProduct = async () => {
    try {
      const response = await fetch(`/api/shop/product/${resolvedParams.slug}`);
  ...
  if (!product) {
    return (
      <div className="flex min-h-[60vh] flex-col items-center justify-center bg-[#fcfcfd] dark:bg-slate-950">
        ...
        <Button variant="ghost" onClick={() => router.push('/shop')} className="mt-4 text-blue-600">
      ...
      <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8 lg:py-12">
        ...
        <div className="flex flex-col gap-12 lg:flex-row lg:items-start lg:gap-16">
          ...
          <div className="flex w-full flex-col lg:w-1/2">
            <div className="mb-8">
              <h1 className="text-3xl font-black leading-tight tracking-tight text-slate-900 dark:text-white sm:text-4xl md:text-5xl">
                {product.productName}
              </h1>
            ...
            <div className="mb-8 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
              <div className="flex flex-wrap items-end justify-between gap-4">
                <div>
                  <p className="mb-1 text-[10px] font-bold uppercase tracking-widest text-slate-400">Unit Price</p>
                ...
                <div>
                  <p className="mb-2 text-right text-[10px] font-bold uppercase tracking-widest text-slate-400">Quantity</p>
              ...
              <div className="flex flex-col gap-3 sm:flex-row">
                <Button
                  ...
                  className={`flex-1 rounded-2xl py-6 text-sm font-bold shadow-lg transition-all ${
                    inCart
                      ? 'cursor-not-allowed bg-emerald-50 text-emerald-600 hover:bg-emerald-50 dark:bg-emerald-900/20 dark:text-emerald-400 shadow-none'
            ...
            <div className="space-y-8">
              {product.productDescription && (
                <div>
                  <h3 className="mb-3 text-sm font-black uppercase tracking-widest text-slate-900 dark:text-white">Overview</h3>
              ...
              {product.productFeature && (
                <div>
                  <h3 className="mb-3 text-sm font-black uppercase tracking-widest text-slate-900 dark:text-white">Key Features</h3>
                  <div className="prose prose-sm max-w-none whitespace-pre-wrap break-words text-slate-600 [overflow-wrap:anywhere] dark:prose-invert">
                    {product.productFeature}
                  </div>
                </div>
              )}
              ...
              {product.productSpecification && (
                <div>
                  <h3 className="mb-3 text-sm font-black uppercase tracking-widest text-slate-900 dark:text-white">Easy Buy</h3>
                  <div className="prose prose-sm max-w-none whitespace-pre-wrap break-words rounded-2xl bg-slate-50 p-6 text-slate-600 [overflow-wrap:anywhere] dark:prose-invert dark:bg-slate-900/50">
                    {product.productSpecification}
                  </div>
                </div>
              )}
              ...
              {relatedProducts.map((relatedProduct) => {
                ...
                return (
                  <div
                    ...
                    onClick={() => router.push(`/shop/${relatedProduct.pidProduct}`)}
