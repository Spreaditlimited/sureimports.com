...
export default function CbmVolumetricWeightCalculatorPage() {
  ...
  const calculated = useMemo(() => {
    ...
    const chargeableKg =
      totalActualKg !== null ? Math.max(totalActualKg, totalVolumetricKg) : totalVolumetricKg;
  ...
  .pillActive {
    ...
    background: linear-gradient(90deg, rgba(37, 99, 235, 0.85), rgba(168, 85, 247, 0.85));
    color: white;
  }
...
`}</style>
      ...
      <p className="sub">
        Use this tool to calculate CBM for sea freight and volumetric weight for air freight using carton dimensions.
        This helps you estimate space and chargeable weight before you buy from China.
      </p>
      ...
      <section className="card" aria-label="Calculator">
          ...
          {(mode === "both" || mode === "air") && (
            <div className="field" style={{ gridColumn: "1 / -1" }}>
              <label className="label">
                Actual weight per carton (kg) <span style={{ fontWeight: 700, opacity: 0.6 }}>(optional)</span>
              </label>
              <input
                className="input"
                inputMode="decimal"
              ...
              />
              <div className="hint">
                Air freight chargeable weight is the higher of actual weight and volumetric weight.
              </div>
            </div>
          )}
        </div>
        ...
        <div className="actions">
          ...
          <button
            ...
            style={{ opacity: canCalculate ? 1 : 0.6, cursor: canCalculate ? "pointer" : "not-allowed" }}
          >
            View Results
          </button>
        </div>
        ...
        <div id="results" className="results" aria-live="polite">
          ...
          {!canCalculate || !calculated ? (
            <div className="meaning">Enter length, width, height, and quantity to see results.</div>
          ) : (
            <div className="resultGrid">
              {(mode === "both" || mode === "sea") && (
                <div className="resultBox">
                  ...
                  <div className="line">
                    <div>
                      <b>CBM per carton:</b> {nf4.format(calculated.cbmPerCarton)} m³
                    </div>
                    <div>
                      <b>Total CBM:</b> {nf4.format(calculated.totalCbm)} m³ ({inputs.Q} cartons)
                    </div>
                  </div>
                  <div className="meaning" style={{ marginTop: 8 }}>
                    CBM measures space. Sea freight is usually priced by CBM, especially for bulky shipments.
                  </div>
                </div>
              )}
              ...
              {(mode === "both" || mode === "air") && (
                <div className="resultBox">
                  <div className="resultBoxTitle">Air freight (Volumetric weight)</div>
                  <div className="line">
                    <div>
                      <b>Volumetric weight per carton:</b> {nf2.format(calculated.volumetricKgPerCarton)} kg
                    </div>
                    <div>
                      <b>Total volumetric weight:</b> {nf2.format(calculated.totalVolumetricKg)} kg ({inputs.Q} cartons)
                    </div>

                    {calculated.totalActualKg !== null && (
                      <div>
                        <b>Total actual weight:</b> {nf2.format(calculated.totalActualKg)} kg
                      </div>
                    )}

                    <div className="divider">
                      <b>Chargeable weight:</b> {nf2.format(calculated.chargeableKg)} kg
                      <div className="meaning" style={{ marginTop: 4 }}>
                        Airlines charge the higher of actual weight and volumetric weight.
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="meaning">
                <b>Quick meaning:</b> Your carton size determines space (CBM). For air freight, large cartons can be
                charged as heavier than they really are because the airline bills by space used.
              </div>
            </div>
          )}
        </div>
        ...
        <div className="note">
          Provided by Sure Imports for education. Results are estimates based on standard formulas, not a freight quote.
        </div>
      </section>
      ...
      <section className="below" aria-label="How it works">
        ...
        <p className="p">
          CBM means Cubic Meter. It tells you how much space your cartons occupy. Sea freight and container planning
          often depends on CBM, especially when the shipment is bulky.
        </p>
        <ul className="ul">
        ...
        <p className="p">
          Volumetric weight is how airlines convert space into weight for billing. If your cartons are big but not heavy,
          you can still be charged as if they were heavy because they take up space on the aircraft.
        </p>
        <p className="p">
          The standard formula used by many forwarders is: (L × W × H) ÷ 6000, where dimensions are in centimeters.
        </p>
        ...
        <div className="faq">
          <div className="qa">
            <p className="q">Why is my air freight chargeable weight higher than my actual weight?</p>
            <p className="a">
              Because airlines charge the higher of actual weight and volumetric weight. If your carton is large, the
              volumetric calculation can exceed your actual kg.
            </p>
          </div>
          ...
          <div className="qa">
            ...
            <p className="a">
              No. CBM is space. Weight is kg. A light but bulky carton can have low kg but high CBM, and it can be
              expensive to ship.
            </p>
          </div>

          <div className="qa">
            <p className="q">Should I use carton dimensions or product dimensions?</p>
            <p className="a">
              Use carton dimensions (the packaged box). Freight pricing is based on the shipped package, not the bare
              product.
            </p>
          </div>

          <div className="qa">
            <p className="q">Can this calculator give me exact shipping cost?</p>
            <p className="a">
              No. It gives you CBM and chargeable weight, which are the numbers you need before requesting a freight
              quote.
            </p>
          </div>
        </div>
