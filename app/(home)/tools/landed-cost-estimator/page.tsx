  ...
  return (
    ...
    >
      ...
      >
        ...
        <span
          style={{ opacity: 0.75, transform: open ? 'rotate(180deg)' : 'none' }}
        >
          ▾
        </span>
      </button>
...
export default function LandedCostEstimatorPage() {
  ...
  const [shippingKey, setShippingKey] = useState<string>(
    shippingOptions[0].key,
  );
  ...
  const selectedShipping = useMemo(() => {
    return (
      shippingOptions.find((o) => o.key === shippingKey) ?? shippingOptions[0]
    );
  ...
  const canCalculate = Boolean(
    u && q && (selectedShipping.basis === 'perKg' ? kg : m3),
  );
  ...
  const result = useMemo(() => {
    ...
    const shippingCost =
      selectedShipping.basis === 'perKg'
        ? selectedShipping.rateUsd * kg!
        : selectedShipping.rateUsd * m3!;
  ...
  async function copyEstimate() {
    ...
    const destLabel =
      DESTINATIONS.find((d) => d.key === destination)?.label ?? destination;
    const basisLine =
      selectedShipping.basis === 'perKg'
        ? `Chargeable weight: ${kg} kg`
        : `Total CBM: ${m3} m³`;
...
`.trim();
  ...
  return (
    ...
    >
      ...
      >
        Estimate total landed cost using all-in rates (shipping, duties, and
        taxes included).
      </p>
        ...
        {selectedShipping.basis === 'perKg' ? (
          <div style={{ display: 'grid', gap: 6 }}>
            ...
            <div style={{ fontSize: 13, opacity: 0.78, lineHeight: 1.55 }}>
              Chargeable weight is the higher of actual weight and volumetric
              weight.
            </div>
          </div>
        ) : (
        ...
        >
          ...
          {!result ? (
            <div style={{ fontSize: 13, opacity: 0.8, lineHeight: 1.6 }}>
              Enter unit price, quantity, and{' '}
              {selectedShipping.basis === 'perKg' ? 'chargeable kg' : 'CBM'} to
              see estimate.
            </div>
          ) : (
            <div style={{ display: 'grid', gap: 8, lineHeight: 1.7 }}>
              ...
              <div>
                <b>All-in shipping (includes duties & taxes):</b> $
                {nf2.format(result.shippingCost)}
              </div>
              <div
                style={{
                  paddingTop: 8,
              ...
              <div>
                <b>Estimated landed cost per unit:</b> $
                {nf2.format(result.perUnit)}
              </div>
              ...
              >
                <div style={{ fontSize: 13, opacity: 0.8 }}>
                  Share or bookmark this for quick planning.
                </div>
                <button
                  type="button"
      ...
      >
        ...
        <p style={{ margin: '0 0 12px', opacity: 0.86, lineHeight: 1.75 }}>
          Landed cost is the real total cost of getting a product to your hands,
          not just the price you paid the supplier. When importing, many people
          focus on the unit price and forget that shipping, duties, taxes, and
          clearance can easily change the final cost per unit. That difference
          is what makes some imports profitable and others a painful mistake.
        </p>
        ...
        <p style={{ margin: '0 0 12px', opacity: 0.86, lineHeight: 1.75 }}>
          In practical terms, your landed cost typically includes your supplier
          cost (product unit price times quantity) plus the shipping plan you
          choose. On Sure Imports all in routes, the shipping rate already
          includes duties and taxes, so your estimate is simpler and more
          predictable. You just need accurate inputs.
        </p>
        ...
        <p style={{ margin: '0 0 12px', opacity: 0.86, lineHeight: 1.75 }}>
          If you price your goods based only on supplier price, you can
          underprice and lose money, or overprice and struggle to sell. Landed
          cost helps you set a realistic selling price, calculate your margin,
          compare shipping methods, and decide whether to import at all. It’s
          also how serious importers compare suppliers fairly because a cheaper
          supplier is not always cheaper after shipping.
        </p>
        ...
        >
          <li>
            Use chargeable weight for air and volumetric based routes, not only
            actual weight.
          </li>
          <li>
            For sea by CBM, total up your carton volume correctly before
            estimating.
          </li>
          <li>
            Always estimate cost per unit so you can price properly and compare
            options.
          </li>
        </ul>
