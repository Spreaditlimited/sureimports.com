...
export default function GeneratorSizingTool() {
  ...
  const result = useMemo(() => {
    ...
    if (phase === "single") {
      warnings.push("Single-phase generators are rarely suitable for factories with industrial motors. If you have any 3-phase machines, use a 3-phase generator.");
    } else {
      warnings.push("If you use a 3-phase generator, phase balancing and correct cable sizing matter as much as generator size.");
    }

    if (startMethod === "DOL") {
      warnings.push("DOL starting creates the highest surge. If your biggest motor is large, consider a soft starter or VFD to reduce starting stress.");
    }

    if ((powerFactor as number) < 0.8) {
      warnings.push("Low power factor reduces usable kW from the generator. If unsure, 0.8 is a common planning value for mixed motor loads.");
    }

    warnings.push("In Nigeria, voltage dips from undersized generators can reset control panels (PLCs), drop contactors, and damage motors over time.");
...
const engineerResult = useMemo(() => {
  ...
  if (engPfFallback === null || engPfFallback <= 0 || engPfFallback > 1) return null;
  if (engHeadroom === null || engHeadroom < 0 || engHeadroom > 80) return null;
  ...
  const startStackKva = startScenario === "largest_only" ? motorStartKva : (baseKva + motorStartKva);
  ...
  if (phase === "single") {
    ...
    warnings.push("3-phase systems require phase balancing, proper ATS sizing, and correct cable sizing. Bad installation can destroy equipment even when generator size is correct.");
  }

  if (largestMotorRow && (largestMotorRow.startMethod ?? "DOL") === "DOL") {
    warnings.push("Your largest motor is set to DOL. Consider soft starter or VFD for large motors to reduce starting surge and voltage dip.");
  }

  warnings.push("Nigeria tip: If your generator is ‘big enough’ but control panels reset, your alternator/AVR response and installation quality are usually the real issue.");
...
function copyEngineerResult() {
  ...
  return (
    <main style={{ maxWidth: 980, margin: "0 auto", padding: "36px 16px 64px", color: "rgba(255,255,255,0.92)" }}>
      ...
      <p style={{ margin: "0 0 22px", opacity: 0.85, lineHeight: 1.7, maxWidth: 900 }}>
        Quick estimate for generator size using running load, power factor, and the starting surge of your largest motor.
        This is designed for real Nigerian factory conditions where voltage dips can reset control panels and damage motors.
      </p>
      ...
      {mode === "quick" && <section className="card">
        <div>
          <div style={labelStyle}>Factory supply type</div>
          <div className="pillRow">
            <button type="button" className={`pill ${phase === "three" ? "pillActive" : ""}`} onClick={() => setPhase("three")}>
              3-Phase
            </button>
            <button type="button" className={`pill ${phase === "single" ? "pillActive" : ""}`} onClick={() => setPhase("single")}>
              Single-Phase
            </button>
          </div>
        </div>
        ...
        <div className="twoCol">
          <div>
            ...
            <input value={runningKw} onChange={(e) => setRunningKw(e.target.value)} inputMode="decimal" placeholder="e.g. 45" style={inputStyle} />
            <div style={{ fontSize: 13, opacity: 0.78, marginTop: 8, lineHeight: 1.6 }}>
              Use the load that will realistically run together, not the theoretical maximum.
            </div>
          </div>

          <div>
            <div style={labelStyle}>Power Factor (PF)</div>
            <input value={pf} onChange={(e) => setPf(e.target.value)} inputMode="decimal" placeholder="e.g. 0.80" style={inputStyle} />
            <div style={{ fontSize: 13, opacity: 0.78, marginTop: 8, lineHeight: 1.6 }}>
              If you don’t know, 0.80 is a safe planning default for mixed motor loads.
            </div>
          </div>
        </div>

        <div style={{ paddingTop: 2 }}>
          <div style={{ fontWeight: 900, marginBottom: 10 }}>Largest Motor (most important for surge)</div>

          <div className="twoCol">
            <div>
              <div style={labelStyle}>Largest motor value</div>
              <input value={largestMotor} onChange={(e) => setLargestMotor(e.target.value)} inputMode="decimal" placeholder="e.g. 7.5" style={inputStyle} />
            </div>

            <div>
              <div style={labelStyle}>Motor unit</div>
              <div className="pillRow">
                <button type="button" className={`pill ${motorUnit === "kW" ? "pillActive" : ""}`} onClick={() => setMotorUnit("kW")}>
                  kW
                </button>
                <button type="button" className={`pill ${motorUnit === "HP" ? "pillActive" : ""}`} onClick={() => setMotorUnit("HP")}>
                  HP
                </button>
              </div>
            </div>
          ...
          <div className="twoCol" style={{ marginTop: 12 }}>
            ...
            <div>
              ...
              <select
                ...
                style={inputStyle}
              >
                <option value="sequential">Sequential (one-by-one with pause)</option>
                <option value="random">Random (operators start anytime)</option>
              </select>
            </div>
          </div>
        </div>
        ...
        <div className="twoCol">
          <div>
            ...
            <input value={headroomPct} onChange={(e) => setHeadroomPct(e.target.value)} inputMode="decimal" placeholder="e.g. 30" style={inputStyle} />
            <div style={{ fontSize: 13, opacity: 0.78, marginTop: 8, lineHeight: 1.6 }}>
              Nigeria tip: 25–35% headroom is sensible for most factories because people add loads and installation quality varies.
            </div>
          </div>
          ...
          >
            ...
            {!result ? (
              <div style={{ fontSize: 13, opacity: 0.8, lineHeight: 1.6 }}>
                Enter running load, PF, and your largest motor to get a generator recommendation.
              </div>
            ) : (
              <div style={{ display: "grid", gap: 8, lineHeight: 1.75 }}>
                ...
                <div>
                  <b>Estimated motor start surge:</b> {nf2.format(result.motorStartKva)} kVA
                </div>

                <div style={{ paddingTop: 8, borderTop: "1px solid rgba(255,255,255,0.10)" }}>
                  <b>Minimum recommended generator:</b> {nf2.format(result.minRecommendedKva)} kVA
                </div>
                <div>
                  <b>Comfortable recommendation:</b> {nf2.format(result.comfortableKva)} kVA
                </div>
                <div style={{ fontSize: 13, opacity: 0.82 }}>
                  Approx usable kW at PF {pf}: {nf2.format(result.approxKwAtPf)} kW
                </div>

                <div style={{ padding: 12, borderRadius: 14, border: "1px solid rgba(255,255,255,0.10)", background: "rgba(7,11,20,0.35)" }}>
        ...
        {result && (
          <div style={{ padding: 14, borderRadius: 16, border: "1px solid rgba(255,255,255,0.10)", background: "rgba(7,11,20,0.30)" }}>
            <div style={{ fontWeight: 900, marginBottom: 8 }}>Practical notes (Nigeria reality)</div>
            <ul style={{ margin: 0, paddingLeft: 18, opacity: 0.86, lineHeight: 1.7 }}>
              {result.warnings.map((w) => (
                <li key={w}>{w}</li>
              ))}
            </ul>
            <div style={{ fontSize: 13, opacity: 0.82, marginTop: 10, lineHeight: 1.6 }}>
              If you’re sourcing industrial machines from China and you want to confirm power requirements, wiring, and motor starting behavior before import,
              Sure Importers Limited can help you plan properly and avoid costly generator and installation mistakes.
              You can also use{" "}
                <a
                ...
                (AI machine sourcing assistant) to qualify your machine specs before you buy.
            </div>
          </div>
        )}
      </section>}
...
{mode === "engineer" && (
  <section className="card">
    <div style={{ fontWeight: 900, fontSize: 18 }}>Engineer Mode (Load Table)</div>
    <div style={{ fontSize: 13, opacity: 0.82, lineHeight: 1.6 }}>
      Add major loads (motors, heaters, compressors, welding, lighting). PF is optional, defaults are applied by type.
      Mark loads that are assumed ON during a motor start.
    </div>

    <div className="twoCol">
      <div>
        <div style={labelStyle}>Start scenario</div>
        <select value={startScenario} onChange={(e) => setStartScenario(e.target.value as any)} style={inputStyle}>
          <option value="largest_plus_base">Largest motor + base load</option>
    ...
    <div className="twoCol">
      <div>
        <div style={labelStyle}>PF fallback (only used for kW estimate display)</div>
        <input
...
>
  ...
  <input
    ...
    style={{ ...inputStyle, padding: "10px 12px", minWidth: 200 }}
  />
</td>
  ...
  <select
    ...
    onChange={(e) => {
      ...
      updateRow(r.id, {
        type: t,
        pf: undefined,
        startMethod: isMotorLike(t) ? (r.startMethod ?? "DOL") : undefined,
        unit: isMotorLike(t) ? r.unit : "kW",
      });
    }}
    style={{ ...inputStyle, padding: "10px 12px", minWidth: 150 }}
  >
    ...
    <option value="OTHER">Other</option>
              ...
              <td style={{ padding: "8px", width: 110 }}>
                <select
                  value={r.unit}
                  onChange={(e) => updateRow(r.id, { unit: e.target.value as any })}
                  style={{ ...inputStyle, padding: "10px 12px" }}
                  disabled={!isMotorLike(r.type)}
                  title={!isMotorLike(r.type) ? "Only motors/compressors support HP input" : undefined}
                >
              ...
              <td style={{ padding: "8px", width: 140 }}>
                <input
                  ...
                  onChange={(e) => updateRow(r.id, { kwOrHp: Number(e.target.value || 0) })}
                  inputMode="decimal"
                  style={{ ...inputStyle, padding: "10px 12px" }}
                />
              </td>
              ...
              <td style={{ padding: "8px", width: 130 }}>
                <input
                  value={r.pf ?? ""}
                  onChange={(e) => updateRow(r.id, { pf: e.target.value ? Number(e.target.value) : undefined })}
                  inputMode="decimal"
              ...
              <td style={{ padding: "8px", width: 170 }}>
                <select
                  ...
                  onChange={(e) => updateRow(r.id, { startMethod: e.target.value as any })}
                  style={{ ...inputStyle, padding: "10px 12px" }}
                  disabled={!isMotorLike(r.type)}
                  title={!isMotorLike(r.type) ? "Only motors/compressors have start method" : undefined}
                >
              ...
              <td style={{ padding: "8px", width: 150 }}>
                <label style={{ display: "flex", gap: 8, alignItems: "center", fontWeight: 900, fontSize: 13, opacity: 0.9 }}>
                  <input
                    type="checkbox"
                    checked={r.includeInStartStack}
                    onChange={(e) => updateRow(r.id, { includeInStartStack: e.target.checked })}
                  />
                  Yes
                </label>
              </td>
    ...
    <div
      ...
      aria-live="polite"
    >
      <div style={{ fontWeight: 900, marginBottom: 10 }}>Engineer Result</div>

      {!engineerResult ? (
        <div style={{ fontSize: 13, opacity: 0.8, lineHeight: 1.6 }}>
          Add loads and ensure headroom/PF values are valid to see recommendations.
        </div>
      ) : (
        <div style={{ display: "grid", gap: 8, lineHeight: 1.75 }}>
          <div>
            <b>Total running load:</b> {nf2.format(engineerResult.totalKw)} kW
          </div>
          <div>
            <b>Total running:</b> {nf2.format(engineerResult.totalKva)} kVA
          </div>

          <div style={{ paddingTop: 8, borderTop: "1px solid rgba(255,255,255,0.10)" }}>
            <b>Largest motor (identified):</b> {engineerResult.largestMotorRow?.name ?? "None"}
          </div>
          <div>
            <b>Largest motor start surge:</b> {nf2.format(engineerResult.motorStartKva)} kVA
          </div>
          <div>
            <b>Start stack kVA:</b> {nf2.format(engineerResult.startStackKva)} kVA
          </div>

          <div style={{ paddingTop: 8, borderTop: "1px solid rgba(255,255,255,0.10)" }}>
            <b>Minimum recommended generator:</b> {nf2.format(engineerResult.minRecommendedKva)} kVA
          </div>
          <div>
            <b>Comfortable recommendation:</b> {nf2.format(engineerResult.comfortableKva)} kVA
          </div>
          <div style={{ fontSize: 13, opacity: 0.82 }}>
            Approx usable kW at PF {engineerPfFallback}: {nf2.format(engineerResult.approxKwAtPf)} kW
          </div>
          ...
          <div style={{ fontSize: 13, opacity: 0.82, lineHeight: 1.6 }}>
            If you’re importing machines from China, validate motor power, phase, and start method before shipping. You can use{" "}
            <a
              href="https://linescout.sureimports.com/machine-sourcing"
              target="_blank"
              rel="noopener noreferrer"
...
>
  ...
  <p style={{ margin: "0 0 12px", opacity: 0.86, lineHeight: 1.75 }}>
    Generator sizing for a factory is not about adding up the “kW” written on machines and buying the next big generator you see.
    Factories are dominated by motor loads: pumps, compressors, conveyors, mixers, mills, blowers, and gear motors. The big problem
    is not the running load. The big problem is what happens when the largest motor starts.
  </p>

  <p style={{ margin: "0 0 12px", opacity: 0.86, lineHeight: 1.75 }}>
    When a motor starts, it can demand a short burst of power that is much higher than its normal running power. If your generator
    cannot supply that burst, you get a voltage dip. In Nigerian factories, voltage dips don’t just “slow the motor.” They often:
    trip contactors, reset PLCs, drop relays, cause nuisance shutdowns, and over time damage motor windings and control boards.
    That’s why many factories have a generator that “runs the load” but cannot start the line reliably.
  </p>

  <p style={{ margin: "0 0 12px", opacity: 0.86, lineHeight: 1.75 }}>
    This calculator uses your realistic running load, power factor, and the largest motor in your plant to estimate a minimum
    generator size in kVA, then adds headroom. Headroom matters in Nigeria because installation quality varies (cable sizing,
    earthing, ATS sizing, phase balancing), and because most factories expand and add loads without re-planning.
  </p>
  ...
  <ul style={{ margin: 0, paddingLeft: 18, opacity: 0.86, lineHeight: 1.75 }}>
    <li><b>Power factor matters:</b> generators are rated in kVA, and motors consume reactive power. If you guess PF too high, you under-size.</li>
    <li><b>Starting method matters:</b> DOL creates the highest surge, star-delta reduces it, soft starters reduce it more, VFD is lowest at start.</li>
    <li><b>Operating behavior matters:</b> if operators start machines randomly, the generator needs more “stiffness” than when you start sequentially.</li>
    <li><b>Headroom matters:</b> Nigerian factories typically need 25–35% margin for expansion, wiring realities, and generator quality differences.</li>
  </ul>
  ...
  <p style={{ margin: "14px 0 0", opacity: 0.86, lineHeight: 1.75 }}>
    If you are sourcing machines from China, confirm power requirements (kW/HP), phase, and starting method before you ship. You can use{" "}
    <a
      href="https://linescout.sureimports.com/machine-sourcing"
      target="_blank"
      rel="noopener noreferrer"
    ...
    to qualify machine specs and reduce generator and installation mistakes.
  </p>
</section>
