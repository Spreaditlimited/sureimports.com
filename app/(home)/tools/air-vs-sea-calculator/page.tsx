...
export default function AirVsSeaCalculator() {
  ...
  const result = useMemo(() => {
    ...
    let recommendation = "";
    if (cheaper === "equal") recommendation = "Air and Sea are the same cost based on your inputs.";
    if (cheaper === "air") {
      recommendation =
        urgent === "yes"
  ...
  return (
    <main style={{ maxWidth: 980, margin: "0 auto", padding: "36px 16px 64px", color: "rgba(255,255,255,0.92)" }}>
      ...
      <p style={{ margin: "0 0 22px", opacity: 0.85, lineHeight: 1.7, maxWidth: 860 }}>
        Enter your shipment weight and volume plus your current rates. This tool compares estimated air and sea shipping costs
        and shows which option makes more financial sense.
      </p>
      ...
      <section className="card">
        <div className="twoCol">
          <div>
            ...
            <input value={kg} onChange={(e) => setKg(e.target.value)} inputMode="decimal" placeholder="e.g. 120" style={inputStyle} />
          </div>
        ...
        <div className="twoCol">
          <div>
            ...
            <input value={airRate} onChange={(e) => setAirRate(e.target.value)} inputMode="decimal" placeholder="e.g. 10" style={inputStyle} />
          </div>

          <div>
            <div className="label">Sea Rate (USD per CBM)</div>
            <input value={seaRate} onChange={(e) => setSeaRate(e.target.value)} inputMode="decimal" placeholder="e.g. 350" style={inputStyle} />
          </div>
        </div>
          ...
          {!result ? (
            ...
            <div style={{ display: "grid", gap: 8, lineHeight: 1.75 }}>
              ...
              <div style={{ paddingTop: 8, borderTop: "1px solid rgba(255,255,255,0.10)" }}>
                ...
                {result.cheaper === "equal" ? "" : `(${result.cheaper.toUpperCase()} is cheaper)`}
              </div>

              {result.closeCall && (
                <div style={{ padding: 12, borderRadius: 14, border: "1px solid rgba(255,255,255,0.10)", background: "rgba(255,255,255,0.04)", opacity: 0.9 }}>
                  <b>Close call:</b> Costs are within ~10%. Delivery time and risk may matter more than price here.
                </div>
              )}
              ...
              <div style={{ fontSize: 13, opacity: 0.82, lineHeight: 1.6 }}>
                <b>Break-even intuition:</b> At your rates, sea becomes cheaper as CBM grows. For your current CBM ({v}), air equals sea at about{" "}
                <b>{nf2.format(result.kgAtGivenCbm)}</b> kg. And for your current kg ({w}), air equals sea at about{" "}
...
>
  ...
  <p style={{ margin: "0 0 12px", opacity: 0.86, lineHeight: 1.75 }}>
    The best shipping method depends on two things: how bulky your cartons are and how fast you need delivery.
    Air freight is usually priced by chargeable weight (kg), while sea freight is often priced by volume (CBM).
    That is why a shipment that looks “light” can still be expensive by air if it is bulky, and why a shipment can be
    affordable by sea once the volume becomes large enough.
  </p>

  <p style={{ margin: "0 0 12px", opacity: 0.86, lineHeight: 1.75 }}>
    This calculator helps you compare both options using your own rates. If the difference is small, then delivery time,
    damage risk, and cashflow can matter more than price. If the difference is large, the cheaper method is usually the
    smarter decision unless you have a strict deadline.
  </p>
  ...
  <ul style={{ margin: 0, paddingLeft: 18, opacity: 0.86, lineHeight: 1.75 }}>
    ...
    <li>If the cartons are bulky and you can wait, sea usually wins on cost.</li>
    <li>If costs are within 10%, choose based on speed and reliability.</li>
  </ul>
