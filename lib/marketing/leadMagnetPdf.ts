...
export function renderLeadMagnetPdfBuffer(input: {
  ...
  if (audience || promise) {
    ...
    y =
      addWrappedText(
        doc,
        [audience, promise].filter(Boolean).join(' - '),
        18,
        y + 7,
        172,
        5,
  ...
  for (const section of sections) {
    ...
    y =
      addWrappedText(doc, clean(section.heading, 'Key points'), 16, y, 178, 6) +
      3;
  ...
  const actionPlan = Array.isArray(pdf.actionPlan)
    ? pdf.actionPlan.slice(0, 6)
    : [];
  if (actionPlan.length) {
    ...
    doc.roundedRect(
      ...
      y - 6,
      182,
      Math.min(70, 14 + actionPlan.length * 8),
      2,
      2,
      'F',
    );
    ...
    actionPlan.forEach((item, index) => {
      y =
        addWrappedText(doc, `${index + 1}. ${clean(item)}`, 20, y, 166, 5) + 2;
    });
    y += 8;
  }
