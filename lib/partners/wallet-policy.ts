...
export function walletTotals(
  credits: { state: string; amountMinor: bigint | string }[],
  withdrawals: { status: string; amountMinor: bigint | string }[],
) {
  const sum = (items: { amountMinor: bigint | string }[]) =>
    items.reduce((total, item) => total + BigInt(item.amountMinor), BigInt(0));
  ...
  const reserved = sum(
    withdrawals.filter((row) =>
      ['REQUESTED', 'PROCESSING', 'OTP_REQUIRED'].includes(row.status),
    ),
  );
  ...
  return {
    ...
    held: held.toString(),
    available: (net > BigInt(0) ? net : BigInt(0)).toString(),
    adjustmentDue: (net < BigInt(0) ? -net : BigInt(0)).toString(),
...
export function walletAmount(value: unknown) {
  ...
  if (
    !/^[1-9]\d{0,14}$/.test(text) ||
    BigInt(text) > BigInt(Number.MAX_SAFE_INTEGER)
  )
    throw new Error('Enter a valid amount in kobo.');
  return BigInt(text);
}
export function transferState(status: string) {
  if (status === 'success') return 'PAID';
  if (status === 'reversed') return 'REVERSED';
  if (['failed', 'abandoned', 'blocked', 'rejected'].includes(status))
    return 'FAILED';
