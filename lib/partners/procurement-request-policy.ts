...
export function isChineseProductLink(value: string) {
  try {
    ...
    return url.protocol === 'https:' && !url.username && !url.password && !url.port &&
      ['1688.com', 'taobao.com', 'tmall.com', 'alibaba.com', 'aliexpress.com', 'jd.com', 'yangkeduo.com', 'pinduoduo.com']
        .some(host => url.hostname === host || url.hostname.endsWith(`.${host}`));
  } catch { return false; }
}
export const procurementRequestSchema = z.object({
  reference: text(3, 80).regex(/^[a-zA-Z0-9][a-zA-Z0-9._-]*$/, 'Use letters, numbers, dots, hyphens or underscores.'),
  title: text(3, 120),
  shipping: z.enum(['AIR', 'SEA', 'ADVISE']),
  products: z.array(z.object({
    link: text(8, 2000).refine(isChineseProductLink, 'Use an HTTPS product link from a supported Chinese marketplace.'),
    name: text(2, 120),
