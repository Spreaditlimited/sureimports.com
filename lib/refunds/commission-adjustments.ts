...
export async function reconcileRefundCommissions() {
  ...
  for (const row of rows) {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      >`SELECT id,affiliateId,commissionCurrency,commissionAmount,status FROM affiliate_conversions WHERE externalOrderReference=${orderReference} ORDER BY id FOR UPDATE`;
      for (const conversion of conversions) {
        if (conversion.status === 'VOIDED') continue;
        const [prior] = await tx.$queryRaw<
          { amount: string | null }[]
        >`SELECT SUM(amount) amount FROM affiliate_refund_adjustments WHERE conversionId=${conversion.id}`;
        const remaining =
          decimalUnits(String(conversion.commissionAmount), 2) -
          decimalUnits(String(prior?.amount || '0'), 2);
        if (remaining < BigInt(0))
          throw new Error('Refund commission reconciliation requires review.');
        const amount = commissionReduction(
          majorAmount(remaining),
          snapshot.productRetentionRatio,
        );
      ...
      const details = JSON.stringify({
        productRetentionRatio: snapshot.productRetentionRatio,
