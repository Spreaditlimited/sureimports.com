...
async function token(app: App) {
  ...
  if (!response.ok || !result.access_token)
    throw new Error('PayPal refund authentication failed.');
...
function resourceUrl(path: string) {
  return (
    base +
    '/v2/' +
    (path.startsWith('/orders/') ? 'checkout' : 'payments') +
    path
  );
}
async function findResource(path: string) {
  for (const app of ['sureimports', 'legacy'] as const) {
    ...
    if (!response.ok || !data.id)
      throw new Error(
        'PayPal has not confirmed this refund. Check its status before retrying.',
      );
    return { access, data };
  }
  throw new Error(
    'The original PayPal payment or refund could not be found. Review its payment reference.',
  );
}
/** Live settlement only. Historical captures use the app that owns them. */
export async function paypalRefundRequest(
  path: string,
  body?: Record<string, unknown>,
  requestId?: string,
) {
  if (!/^\/(captures|refunds|orders)\/[A-Za-z0-9]+(\/refund)?$/.test(path))
    throw new Error('Invalid PayPal resource.');
  if (!body) return (await findResource(path)).data;
  if (!/^\/captures\/[A-Za-z0-9]+\/refund$/.test(path) || !requestId)
    throw new Error(
      'A capture and immutable refund request reference are required.',
    );
  const capturePath = path.slice(0, -7);
  const owner = await findResource(capturePath);
  if (owner.data.id !== capturePath.split('/')[2])
