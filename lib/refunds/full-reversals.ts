...
export async function reconcilePaidCommissionReversals() {
  return prisma.$transaction(async (tx) => {
    ...
    for (const row of rows) {
      ...
      >`SELECT SUM(amount) amount FROM affiliate_refund_adjustments WHERE conversionId=${row.id}`;
      const remaining =
        decimalUnits(String(row.commissionAmount), 2) -
        decimalUnits(String(prior?.amount || '0'), 2);
      if (remaining < BigInt(0))
        throw new Error('Commission recovery requires review.');
