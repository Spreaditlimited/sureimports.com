...
export async function recordExternalPayPalRefund(
  reference: string,
...
export async function closeUnpaidExternalPayPalRefund(
  reference: string,
  adminId: string,
) {
  if (!/^[A-Za-z0-9]{5,50}$/.test(reference))
    throw new Error('Enter a valid PayPal refund reference.');
  const provider = await paypalRefundRequest('/refunds/' + reference);
  const captureId = paypalCaptureReference(
  ...
  if (
    ...
    !captureId ||
    !['FAILED', 'CANCELLED'].includes(provider.status)
  )
    throw new Error(
      'PayPal has not confirmed this refund as failed or cancelled. The review remains open.',
    );
  return prisma.$transaction(async (tx) => {
    const id = 'PP_EXTERNAL:' + reference;
    const [event] = await tx.$queryRaw<
      Array<{ detailsJson: string }>
    >`SELECT detailsJson FROM refund_events WHERE id=${id} AND eventType='EXTERNAL_PAYPAL_REFUND' FOR UPDATE`;
    if (!event) throw new Error('Refund review not found.');
    const evidence = JSON.parse(event.detailsJson);
    if (
      evidence.captureId !== captureId ||
      evidence.currency !== provider.amount?.currency_code ||
      decimalUnits(evidence.amount, 2) !==
        decimalUnits(String(provider.amount?.value || ''), 2)
    )
    ...
    if (settled.length)
      throw new Error(
        'This refund already has a settlement allocation. Review that settlement instead.',
      );
    const existing = await tx.$queryRaw<
      Array<{ eventType: string }>
    >`SELECT eventType FROM refund_events WHERE id=${'RESOLVED:' + id}`;
    if (existing.length) {
      if (existing[0].eventType !== 'EXTERNAL_REFUND_NOT_PAID')
        throw new Error('This refund has already been reconciled differently.');
      return {
        message: 'This failed or cancelled refund review is already closed.',
      };
    }
    ...
    return {
      message:
        'PayPal confirmed that no refund was completed. This review is closed; other payment or refund holds are unchanged.',
    };
  });
}

/** Link a provider-completed refund to an existing, classified procurement entitlement. */
export async function linkExternalPayPalRefund(
  reference: string,
  refundId: string,
  adminId: string,
) {
  if (
    ...
    throw new Error('Enter the existing refund reference.');
  const provider = await paypalRefundRequest(`/refunds/${reference}`);
  const captureId = paypalCaptureReference(
    'PAYMENT.CAPTURE.REFUNDED',
    provider,
  ...
  return prisma.$transaction(async (tx) => {
    ...
    >`SELECT pidUser,pidOrder,currency,amount,refundStatus,ext1,ext2,serviceType FROM refund_records WHERE pidRefund=${refundId} FOR UPDATE`;
    ...
    if (resolved.length) {
      if (resolved[0].refundId !== refundId)
        throw new Error(
          'This PayPal refund is already linked to a different refund.',
        );
    ...
    if (
      !event ||
      !refund ||
      !['pending', 'requested'].includes(refund.refundStatus) ||
      refund.currency !== provider.amount?.currency_code ||
      providerAmount <= BigInt(0) ||
      decimalUnits(refund.amount, 2) !== providerAmount
    )
      throw new Error(
        'The existing refund must match the PayPal currency and amount exactly and must not already be paid.',
      );
    const evidence = JSON.parse(event.detailsJson);
    if (
      evidence.captureId !== captureId ||
      evidence.currency !== refund.currency ||
      decimalUnits(evidence.amount, 2) !== providerAmount
    )
      throw new Error('The PayPal refund does not match the recorded review.');
    let components;
    try {
      components = JSON.parse(refund.ext2);
    } catch {
      components = null;
    }
    if (
      refund.serviceType !== 'PROCUREMENT' ||
      refund.ext1 !== 'ORDER_ADJUSTMENT' ||
      components?.version !== 1 ||
      !/^(?:0\.\d{8}|1\.00000000)$/.test(
        String(components.productRetentionRatio),
      )
    ) {
      throw new Error(
        'This refund needs a product and shipping adjustment breakdown before it can be reconciled.',
      );
    }
    const payments = await tx.$queryRaw<
      Array<{ pidPayment: string; amount: string; currency: string }>
    >`SELECT pidPayment,amount,currency FROM payments WHERE txID=${captureId} AND pidUser=${refund.pidUser} AND serviceID=${refund.pidOrder} AND paymentType='PAYPAL' AND paymentStatus='PAID' ORDER BY id FOR UPDATE`;
    if (
      payments.length !== 1 ||
      payments[0].currency !== refund.currency ||
      decimalUnits(String(payments[0].amount), 2) < providerAmount
    )
      throw new Error(
        'The original payment does not belong to this customer and order.',
      );
    ...
    const legs = await tx.$queryRaw<
      Array<{ id: string }>
    >`SELECT id FROM refund_provider_legs WHERE refundId=${refundId} OR providerReference=${reference}`;
    if (snapshots.length || legs.length)
      throw new Error(
        'This refund already has a settlement request. Review it before linking an external refund.',
      );
    const [reserved] = await tx.$queryRaw<
      Array<{ amount: string | null }>
    >`SELECT SUM(amount) amount FROM refund_provider_legs WHERE captureId=${captureId} AND status<>'SUPERSEDED'`;
    if (
      decimalUnits(String(reserved?.amount || '0'), 2) + providerAmount >
      decimalUnits(String(payments[0].amount), 2)
    )
      throw new Error(
        'Other refunds already use the remaining amount of this payment.',
      );
    ...
    return {
      message:
        'Existing PayPal refund linked. No new refund was sent. Earnings reconciliation and customer notification will follow.',
    };
  });
}
